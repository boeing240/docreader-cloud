...
static PDFIUM_DLL_PATH: Lazy<Result<PathBuf, String>> = Lazy::new(|| {
    let dir = std::env::temp_dir().join("pdf-reader-cloud");
...
impl PdfRenderer {
    ...
    pub fn load_document(&self, path: &Path) -> Result<Vec<u8>> {
        // Just read bytes, actual loading happens per-operation
        std::fs::read(path).context("Failed to read PDF file")
    }

    pub fn get_page_count(&self, bytes: &[u8]) -> Result<u32> {
        let document = self
            .pdfium
    ...
    pub fn render_page(&self, bytes: &[u8], page_index: u32, scale: f32) -> Result<RgbaImage> {
        let document = self
            .pdfium
            .load_pdf_from_byte_slice(bytes, None)
            .context("Failed to load PDF")?;
...
// Helper struct to hold document data
#[allow(dead_code)]
...
#[allow(dead_code)]
impl LoadedDocument {
    pub fn new(renderer: &PdfRenderer, path: &Path) -> Result<Self> {
        let bytes = renderer.load_document(path)?;
        let page_count = renderer.get_page_count(&bytes)?;
