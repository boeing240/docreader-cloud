...
Проект использует GitHub Flow. Основная ветка — `main`.

1. Для любой задачи создавать **feature-ветку** от `main`
2. Коммитить изменения в feature-ветку
3. Открывать Pull Request в `main`
4. После ревью — мержить PR и удалять feature-ветку
5. Релизы: тег `vX.Y.Z` на коммите в `main`

Формат коммитов: на русском, краткое описание изменений.
